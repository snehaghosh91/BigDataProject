...
if __name__ == "__main__":
	sc = SparkContext()
	col = fetch_column(sc, 12)
	col_validity_map = col.map(lambda x : (x[0], validate(x[1], "JURIS_DESC")))
	...
	header = sc.parallelize(["CMPLNT_NUM \t JURIS_DESC"])
	sc.union([header, invalid_col_out]).saveAsTextFile("col12_invalid_data.out")
	valid_col = col_validity_map.filter(lambda x : x[1][1])
	valid_col_out = valid_col.map(lambda x : str(x[0]) + "\t" + str(x[1][0]))
	sc.union([header, valid_col_out]).saveAsTextFile("col12_valid_data.out")
	...
	if not invalid_col.isEmpty():
		...
		sc.union([counts, invalid_max_occ_out]).saveAsTextFile("col12_statistics.out")
	else:
		counts.saveAsTextFile("col12_statistics.out")
	sc.stop()
