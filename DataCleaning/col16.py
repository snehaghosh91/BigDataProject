...
if __name__ == "__main__":
	sc = SparkContext()
	col = fetch_column(sc, 16)
	col_validity_map = col.map(lambda x : (x[0], validate(x[1], "PREM_TYPE_DESC")))
	...
	header = sc.parallelize(["CMPLNT_NUM \t PREM_TYP_DESC"])
	sc.union([header, invalid_col_out]).saveAsTextFile("col16_invalid_data.out")
	valid_col = col_validity_map.filter(lambda x : x[1][1])
	valid_col_out = valid_col.map(lambda x : str(x[0]) + "\t" + str(x[1][0]))
	sc.union([header, valid_col_out]).saveAsTextFile("col16_valid_data.out")
	...
	if not invalid_col.isEmpty():
		...
		sc.union([counts, invalid_max_occ_out]).saveAsTextFile("col16_statistics.out")
	else:
		counts.saveAsTextFile("col16_statistics.out")
	sc.stop()
