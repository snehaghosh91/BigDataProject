...
if __name__ == "__main__":
	...
	#col.distinct().saveAsTextFile("col17_distinct.out")
	...
	header = sc.parallelize(["CMPLNT_NUM \t PARKS_NM"])
	sc.union([header, invalid_col_out]).saveAsTextFile("col17_invalid_data.out")
	valid_col = col_validity_map.filter(lambda x : x[1][1])
	valid_col_out = valid_col.map(lambda x : str(x[0]) + "\t" + str(x[1][0]))
	sc.union([header, valid_col_out]).saveAsTextFile("col17_valid_data.out")
	counts = sc.parallelize(["INVALID COUNT:\t" + str(invalid_col.count()), "VALID COUNT:\t" + str(valid_col.count())]);
		...
		sc.union([counts, invalid_max_occ_out]).saveAsTextFile("col17_statistics.out")
	else:
		counts.saveAsTextFile("col17_statistics.out")
	sc.stop()
